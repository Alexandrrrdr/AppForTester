package com.example.appfortester.utils

object Constants {
    const val MAIN_URL = "https://filetransfer.io/data-package/99TeGwHP/download"
    const val REQUEST_CODE = 100
    const val PACKAGE = "package"
    const val FILE_NAME = "testTextOnly.apk"
    const val PACKAGE_NAME = "com.example.appfortester"
    const val MIME_TYPE = "application/vnd.android.package-archive"
    const val FILE_BASE_PATH = "file://"
    const val PROVIDER_PATH = ".provider"
    const val INTENT_INSTALL = 1
    const val FIREBASE = "eB5CrthsQ96aIFfQhHh3j-:APA91bGM9dLTQwkSaohVOYpVA7fIt1hidId29q5IWJkcF3E78RyHnhxEy_RMN1v8qif7KmcVylGKfL3RbxL0xd2vrQ47m52G_6r_7vOvTZonlTSAn8PjV1ypzY9peY0zzWTmR7CWYnvl"
    const val INTENT_ACTION_SEND = "ACTION_SEND"
    const val INTENT_EXTRA = "com.example.appfortester"
    const val PACKAGE_INSTALL = 2
    const val PERMISSION_REQUEST_STORAGE = 0
    const val APP_INSTALL_PATH = "\"application/vnd.android.package-archive\""
    const val CHANNEL_ID = "notification_channel"
    const val CHANNEL_NAME = "com.example.appfortester"
}